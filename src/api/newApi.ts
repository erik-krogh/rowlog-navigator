...
async function fetchMemberRawData(): Promise<Member[]> {
  ...
  const url = "https://asr.rokort.dk/admin/GetMemberGridDataRaw?sidx=TotalLogins&sord=desc&page=1&rows=100000&departmentFilter=&tagFilter=&notTagFilter=&searchText=&customFilter=Status-,MailDeliveryStatus-";
  const resp = await got.get(url, {
    headers: await authHeader(),
  });

  const rawMembers: RawMemberRow[] = JSON.parse(resp.body).rows.map((r: any) => r.cell);
...
export const tags: () => Promise<Record<string, TagData>> = util.cache(async () => {
...
async function fetchRawTrips(): Promise<RawTrip[]> {
  const url = "https://asr.rokort.dk/api/report/CR2517138924777463952";

  const rawData: RawTrip[] = JSON.parse((await got.get(url, {
    headers: await authHeader(),
  })).body).data;
